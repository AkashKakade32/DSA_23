...
class DirectedGraph: 
    @staticmethod 
    def edge_sorter(E: list[tuple[int, int, float]]):
        for j in range(1, len(E)): 
            key = E[j]
            i = j - 1 
            while i > -1 and E[i][2] > key[2]: 
                E[i+1] = E[i]
                i = i - 1 
            E[i+1] = key 
        return E 
    ...
    def get_edges(self): 
        # You should return a built in list containing 
        # as many tuples as number of edges and each 
        # tuple will contain three numbers, 
        # index 0: v_start, index 1: v_end, index 2: weight 
        E = [] #local list for edges 
        v_run = self.v_list.head_node.next 
        while v_run is not self.v_list.head_node: 
            h_run = v_run.h_list.head_node.next 
            while h_run is not v_run.h_list.head_node: 
                E.append((v_run.v, h_run.v, h_run.w))
                h_run = h_run.next
            v_run = v_run.next 
        return E 
    ...
    def __iter__(self): 
        def get_generator(): 
            ...
            while v_run is not self.v_list.head_node: 
                h_run = v_run.h_list.head_node.next 
                while h_run is not v_run.h_list.head_node: 
                    yield (v_run.v, h_run.v, h_run.w)
                    h_run = h_run.next
                v_run = v_run.next 
        return DirectedGraph_iterator(get_generator())
...
if __name__ == '__main__': 
    ...
    print('PRINTING EDGES OF GRAPH USING ITERATOR')
    for e in dg: 
        print(e)
        

    '''
    print('PRINTING UNSORTED EDGES:')
    E = dg.get_edges()
    for (v_start, v_end, w) in dg.get_edges(): 
        print(f'EDGE:, ({v_start}, {v_end}, {w})')
    
    print('----------------------------')

    print('PRINTING SORTED EDGES:')
    for (v_start, v_end, w) in DirectedGraph.edge_sorter(dg.get_edges()): 
        print(f'EDGE:, ({v_start}, {v_end}, {w})')
