...
class DirectedGraph: 
    ...
    def remove_vertex(self, v: int) -> None: 
        ...
        while h_run is not vnode_of_v.h_list.head_node: 
            h_run_next = h_run.next
            vnode_of_v_in_h_run = vlist.search_node(self.v_list.head_node, h_run.v)
            hnode_of_v_in_adj_list = hlist.search_node(vnode_of_v_in_h_run.h_list.head_node, v)
            if hnode_of_v_in_adj_list is not None: 
                hlist.generic_delete(hnode_of_v_in_adj_list)
            del h_run 
            h_run = h_run_next
            self.nr_edges -= 1 
