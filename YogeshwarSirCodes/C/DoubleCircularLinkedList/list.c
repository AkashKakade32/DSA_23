...
len_t get_list_length(list_t* p_list)
{
    ...
    for(
        p_run = p_list->next, len = 0; 
        p_run != p_list; 
        p_run = p_run->next, ++len
    )
        ; 
    return (len);  
}
...
/* list auxillary routines -> implementation */ 
...
/* auxillary routines -> implementation */ 
static void* xcalloc(
                size_t nr_elements, 
            ...
            )
{
    ...
    if(p == NULL)
    {
        fprintf(
            stderr, 
            "calloc:fatal:out of virtual memory\n"
        ); 
        exit(EXIT_FAILURE); 
    }
