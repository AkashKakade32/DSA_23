...
class graph:  
    ...
    def dijkstra(self, s: int): 
        ...
        v_node_of_s = vlist.search_node(self.v_list.head_node, s) 
        if v_node_of_s is None:
            raise VertexInvalidError(f"Source vertex {s} does not exist")
        
        def initialize_single_source(v_node_of_s: vnode) -> None: 
            v_run = self.v_list.head_node.next 
            while v_run != self.v_list.head_node: 
                v_run.d = math.inf
                v_run.pred_vertex_vnode = None 
                v_run = v_run.next 
            v_node_of_s.d = 0.0

        def relax(source_vnode: vnode, dest_vnode: vnode, w: float) -> None: 
            if dest_vnode.d > source_vnode.d + w: 
                dest_vnode.d = source_vnode.d + w 
                dest_vnode.pred_vertex_vnode = source_vnode
        
        def extract_min(Q: list) -> vnode: 
            min_d = Q[0].d
            min_index = 0 
        ...
        def print_shortest_path(s:int, dest_vnode: vnode): 
            L = []
            run = dest_vnode
            while run is not None: 
                L.append(run.v) 
                run = run.pred_vertex_vnode
            L.reverse() 
            print(f'shortest path from {s} to {dest_vnode.v}')
            print("[START]->", end='')
            for v in L: 
                print(f'[{v}]->', end='')
            print('[END]')
        ...
        initialize_single_source(v_node_of_s)
        ...
        while len(Q) != 0: 
            u = extract_min(Q)
            # for v in adj(u)
            h_run = u.adj_list.head_node.next 
            while h_run != u.adj_list.head_node: 
                w = h_run.w 
                vnode_dest = vlist.search_node(self.v_list.head_node, h_run.v) 
                # Relax(u, v, w)
                relax(u, vnode_dest, w)
                h_run = h_run.next 

        v_run = self.v_list.head_node.next 
        while v_run != self.v_list.head_node:
            print_shortest_path(s, v_run)
            v_run = v_run.next 
...
if __name__ == '__main__':     
    ...
    E = [
        (1, 2, 0.5), (2, 3, 1.8), (3, 4, 2.0), (4, 1, 1.0), 
        (1, 5, 1.3), (2, 5, 0.7), (3, 5, 0.4), (4, 5, 1.2)
    ]
    ...
    for v in range(1, 6) : 
        print(f"---------SHORTEST PATH-SOURCE VERTEX-{v}--------")
        g.dijkstra(v)
    print("------------------------END------------------------")