...
class graph:  
    ...
    def prims(self, r: int): 
        ...
        v_node_of_r = vlist.search_node(self.v_list.head_node, r) 
        if v_node_of_r is None:
            raise VertexInvalidError(f"Source vertex {r} does not exist")
        ...
        while len(Q) != 0: 
            ...
            while h_run != u.adj_list.head_node: 
                ...
                if vnode_dest in Q and w < vnode_dest.key: 
                    vnode_dest.key = w 
                    vnode_dest.pred_vertex_vnode = u
                h_run = h_run.next 
        ...
        while v_run != self.v_list.head_node: 
            if v_run.v != r: 
                A.append((v_run.v, v_run.pred_vertex_vnode.v))
            v_run = v_run.next

        print(f"Edges in the spanning Tree with root vertex {r}:")
        for (u, v) in A: 
            print(f"E:({u}, {v})")
        print("-" * 80)
